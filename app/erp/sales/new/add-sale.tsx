...
export default function AddSale() {
  ...
  const [saleDate, setSaleDate] = useState<string>(new Date().toISOString());
  ...
  async function loadSale(id: string) {
    ...
    const row = data.row;
    setSaleDate(row.saleDate || new Date().toISOString());
    ...
    setPaymentAmount(money(row.paidAmount));
    ...
    try {
      ...
      if (!res.ok) {
        const map: Record<string, string> = {
          ...
          PAYMENT_METHOD_REQUIRED: "Payment method required for paid sales",
  ...
  return (
    <div className="p-6 ">
      <div className="w-full max-w-5xl space-y-6">
        <div>
          <div className="text-2xl font-semibold text-slate-900">{isEditing ? "Edit Sale" : "Add Sale"}</div>
          <div className="text-sm text-slate-500">{isEditing ? "Update sales invoice details." : "Create sales invoices that work with customer due and stock."}</div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              <div className="text-xs mb-1 text-slate-500">Sale Date *</div>
              <input
                className={inputBase}
                type="datetime-local"
                value={new Date(saleDate).toISOString().slice(0, 16)}
                onChange={(e) => setSaleDate(new Date(e.target.value).toISOString())}
              />
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
          ...
          <div className="mt-4 bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <div className="text-xs mb-1 text-slate-500">Payment Amount</div>
                <input
                  className={inputBase}
                  type="number"
                  min={0}
              ...
              <div>
                <div className="text-xs mb-1 text-slate-500">Payment Method</div>
              ...
              <div>
                <div className="text-xs mb-1 text-slate-500">Payment Reference</div>
                <input
                  className={inputBase}
              ...
              <div className="md:col-span-3">
                <div className="text-xs mb-1 text-slate-500">Payment Note</div>
                <input
                  className={inputBase}
                  value={paymentNote}
              ...
              <div className="md:col-span-3 text-sm text-slate-700 space-y-1 text-right">
                <div><b>Paid:</b> Rs {money(Math.min(paymentAmount, grandTotal))}</div>
                <div><b>Due:</b> Rs {money(Math.max(0, grandTotal - paymentAmount))}</div>
                {status === "DRAFT" && paymentAmount > 0 ? (
                  <div className="text-xs text-slate-500">Payments are reserved and will be applied when this sale is finalized.</div>
                ) : null}
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
          ...
          <div className="mt-2 text-xs text-slate-500">
            <b>Draft:</b> no stock or due posting. <b>Final:</b> stock decreases and customer sale due increases.
          </div>
        </div>
      </div>
    </div>
