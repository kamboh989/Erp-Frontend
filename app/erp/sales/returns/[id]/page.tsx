...
export default function SaleReturnDetailPage() {
  ...
  const [row, setRow] = useState<SaleReturnRow | null>(null);
  ...
  async function onCancel() {
    ...
    if (!confirm("Cancel this sale return? This will reverse stock + customer refund due.")) return;
    ...
    <div className="p-6 w-full">
      <style jsx global>{`
        @media print {
          ...
          #return-print, #return-print * { visibility: visible !important; }
          #return-print { position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; padding: 0 !important; margin: 0 !important; }
      ...
      <div className="w-full max-w-5xl space-y-6">
        <div className="flex items-start justify-between gap-4 no-print">
          <div>
            <div className="text-2xl font-semibold text-slate-900">Sale Return Detail</div>
        ...
        <div id="return-print" className="space-y-6">
          <div className="hidden print:block">
            <div className="text-lg font-semibold">Sale Return Detail</div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="text-sm text-slate-700 space-y-1">
                <div><b>Date:</b> {fmtPK(row.returnDate)}</div>
              ...
              <div className="text-sm text-slate-700 space-y-1">
                ...
                <div><b>Refund:</b> {row.paymentStatus || "UNREFUNDED"}</div>
                <div><b>Refunded:</b> Rs {money(row.paidAmount)}</div>
                <div><b>Pending Refund:</b> Rs {money(row.dueAmount)}</div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-x-auto">
            <table className="min-w-[900px] w-full text-sm">
              ...
              <tbody>
                {(row.items || []).map((it: SaleReturnItem, idx: number) => (
          ...
          {row.payments?.length ? (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-x-auto">
              <div className="px-5 py-4 text-sm font-semibold text-slate-800">Refund Details</div>
              <table className="min-w-[700px] w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wide">Refunded At</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wide">Note</th>
                  </tr>
                </thead>
                <tbody>
                  {row.payments.map((p: SaleReturnPayment, idx: number) => (
                    <tr key={idx} className="border-t border-slate-100 hover:bg-slate-50">
