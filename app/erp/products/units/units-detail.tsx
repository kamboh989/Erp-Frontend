...
type ColKey = "name" | "short" | "allowDecimal" | "createdAt";
...
export default function UnitsPage() {
  ...
  // ✅ Action + visible columns count (for colSpan)
  const visibleCount = 1 + (Object.keys(cols) as ColKey[]).filter((k) => cols[k]).length;

  const exportCsvUrl = useMemo(() => {
    const sp = new URLSearchParams();
    if (q.trim()) sp.set("q", q.trim());
    return `/api/erp/units/export/csv?${sp.toString()}`;
  }, [q]);

  const exportExcelUrl = useMemo(() => {
    const sp = new URLSearchParams();
    if (q.trim()) sp.set("q", q.trim());
    return `/api/erp/units/export/excel?${sp.toString()}`;
  }, [q]);
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch(`/api/erp/units?${sp.toString()}`, { cache: "no-store" });
  ...
  async function onDelete(id: string) {
    ...
    const res = await fetch(`/api/erp/units/${id}`, { method: "DELETE" });
    ...
    <div className="p-6 relative w-full">
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-2xl font-semibold text-slate-900">Units</div>
            <div className="text-sm text-slate-500">Manage your units</div>
          </div>
              ...
              {colsOpen && (
                <div className="absolute right-0 z-[9999] mt-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-lg p-3">
                  ...
                  <div className="pt-3 flex gap-2">
                    <button className={pillBtn} onClick={() => setCols({ name: true, short: true, allowDecimal: true, createdAt: false })}>
                      Reset
                    </button>
        ...
        {/* PRINT AREA */}
        <div id="units-print" className="mt-6">
              ...
              <table className="min-w-full w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    {cols.short && <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wide">Short name</th>}
                    {cols.allowDecimal && (
                      <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wide">Allow decimal</th>
                    )}
                ...
                {loading ? (
                  ...
                  rows.map((r) => (
                    <tr key={r._id} className="border-t border-slate-100 hover:bg-slate-50 transition">
                          ...
                          {actionOpenId === r._id && (
                            ...
                            >
                              <button
                                ...
                                onClick={() => {
                                  setActionOpenId(null);
                                  window.location.href = `/erp/products/units/${r._id}`;
                                }}
                              >
                                👁 View
                              </button>
                      ...
                      {cols.short && <td className="px-4 py-3 text-slate-700 whitespace-nowrap">{r.short}</td>}
                      {cols.allowDecimal && <td className="px-4 py-3 text-slate-700 whitespace-nowrap">{r.allowDecimal ? "Yes" : "No"}</td>}
...
function UnitModal({ initial, onClose, onSaved }: { initial: any | null; onClose: () => void; onSaved: () => void }) {
  const [name, setName] = useState(initial?.name || "");
  const [short, setShort] = useState(initial?.short || "");
  const [allowDecimal, setAllowDecimal] = useState(Boolean(initial?.allowDecimal));
  ...
  async function save() {
    ...
    const payload = { name, short, allowDecimal };

    try {
      const res = await fetch(initial?._id ? `/api/erp/units/${initial._id}` : "/api/erp/units", {
      ...
      if (!res.ok) {
        const map: any = {
          NAME_SHORT_REQUIRED: "Name and short name required",
    ...
    >
      <div className="w-full max-w-md sm:max-w-lg md:max-w-xl bg-white rounded-2xl border border-slate-200 shadow-xl">
        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
          <div className="text-lg font-semibold text-slate-900">{initial?._id ? "Edit Unit" : "Add Unit"}</div>
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            ...
            <input className={inputBase} value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Piece" />
          </div>

          <div>
            <div className="text-xs mb-1 text-slate-500">Short name *</div>
            <input className={inputBase} value={short} onChange={(e) => setShort(e.target.value)} placeholder="e.g. pcs" />
          </div>
