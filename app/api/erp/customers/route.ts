...
import { nextContactId } from "@/lib/idGen";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const limit = Math.min(
      100,
      Math.max(10, toNumber(url.searchParams.get("limit"), 25)),
    );
    ...
    const customerGroupId = url.searchParams.get("customerGroupId");
    const assignedTo = url.searchParams.get("assignedTo"); // userId
    const hasNoSellFrom = url.searchParams.get("hasNoSellFrom"); // placeholder

    const sellDue = url.searchParams.get("sellDue") === "1";
    const sellReturn = url.searchParams.get("sellReturn") === "1";
    ...
    const base: any = {
      ...
      contactType: "CUSTOMER", // ✅ ONLY CUSTOMER
    };
    ...
    if (customerGroupId) base.customerGroupId = customerGroupId;
    ...
    // due filters
    if (sellDue) base["totals.totalSaleDue"] = { $gt: 0 };
    if (sellReturn) base["totals.totalSaleReturnDue"] = { $gt: 0 };
    if (advanceBalance) base["totals.advanceBalance"] = { $gt: 0 };
    if (openingBalance) base["totals.openingBalanceDue"] = { $gt: 0 };

    if (hasNoSellFrom) {
      // TODO: implement when Sales/Invoices collection is ready
    }
    ...
    const [rows, total, totalsAgg] = await Promise.all([
      ...
      Contact.aggregate([
        { $match: filter },
        {
          $group: {
            _id: null,
            totalSaleDue: { $sum: "$totals.totalSaleDue" },
            totalSaleReturnDue: { $sum: "$totals.totalSaleReturnDue" },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const partyType = String(body.partyType); // INDIVIDUAL/BUSINESS
    ...
    // ✅ DUPLICATE BLOCK (same company, CUSTOMER only)
    const mobileExists = await Contact.findOne({
      companyId: session.companyId,
    ...
    // contactId hidden auto
    const contactId = await nextContactId(
      String(session.companyId),
      "CONTACT_CUSTOMER",
    );

    // ✅ RBAC SAFE assignedTo
    let assigned: string[] = [];
    if (isCompanyAdmin(session)) {
      assigned = Array.isArray(body.assignedTo) ? body.assignedTo : [];
    } else {
      assigned = [session.userId];
    }

    // ✅ Opening balance lives in moreInfo + totals.openingBalanceDue
    ...
    const doc = await Contact.create({
      ...
      moreInfo: {
        ...
        creditLimit:
          moreInfo.creditLimit === ""
            ? null
            : Number(moreInfo.creditLimit ?? 0) || 0,
        payTerm: String(moreInfo.payTerm || "").trim(),
        openingBalance: openingBal,

        // keep any other nested structures (shipping/billing etc)
        ...moreInfo,
      },
      ...
      contactPersons: Array.isArray(body.contactPersons)
    ...
    // ✅ if DB unique index throws duplicate error
    if (err?.code === 11000) {
      // keys can vary by index; keep generic
