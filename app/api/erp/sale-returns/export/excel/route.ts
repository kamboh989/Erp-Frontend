...
export async function GET(req: NextRequest) {
  try {
    ...
    const headers = ["Date", "Reference No", "Customer", "Location", "Status", "Grand Total", "Added By", "Created At"];
    const lines = [
      ...
      ...rows.map((row: any) => {
        ...
        return [
          row.returnDate ? new Date(row.returnDate).toISOString() : "",
          row.referenceNo,
    ...
    return new NextResponse(lines.join("\n"), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="sale_returns.csv"`,
      },
    });
  } catch (err) {
