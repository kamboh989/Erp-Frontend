...
export async function GET(req: NextRequest) {
  try {
    ...
    const [rows, total, totalsAgg] = await Promise.all([
      ...
      Sale.aggregate([{ $match: filter }, { $group: { _id: null, grandTotal: { $sum: "$grandTotal" } } }]),
    ]);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (paymentAmount > 0 && !paymentMethod) return NextResponse.json({ error: "PAYMENT_METHOD_REQUIRED" }, { status: 400 });
    ...
    const paidAmount = Math.min(paymentAmount, grandTotal);
    const dueAmount = Math.max(0, grandTotal - paidAmount);
    const paymentStatus = paidAmount >= grandTotal ? "PAID" : paidAmount > 0 ? "PARTIAL" : "UNPAID";
    const paymentMethodSnapshot = paidAmount > 0 ? (paymentMethod || "Cash") : "";
